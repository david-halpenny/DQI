...
from utils.params import p, m, n, field_p, Fs, B, r, omega
import numpy as np


def obj_f(index, input):
    """
...
def obj_g(index, input):
    ...
    return (obj_f(index, input) - f_bar)/ varphi

def fourier_g(index, input):
    """
    ...
    g_values = np.array([obj_g(index, x) for x in field_p]) 
...
def objective_value(x_vec):
    ...
    results = [obj_f(i+1, x_vec[i]) for i in range(len(x_vec))]
